export function nested_to_graph(data,nodeName,nodeDesc){
...
function dfs(curr,edges,nodes,isRoot)
{
    ...
    //node bnao
    nodes.add(JSON.stringify({id:curr.concept_id,concept_id:curr.concept_id,concept_name:curr[data_name],concept_desc:curr[data_desc],height: 100,width: 100}))  
    ...
    for(let i=0;i<curr.predecessors.length;i++)
    {
        ...
        edges.add(JSON.stringify({id:`${child.concept_id}-${curr.concept_id}`,from:child.concept_id,to:curr.concept_id}))
    ...
    for(let i=0;i<curr.successors.length;i++)
    {
        ...
        edges.add(JSON.stringify({id:`${curr.concept_id}-${child.concept_id}`,from:curr.concept_id,to:child.concept_id}))
    ...
    for(let i=0;i<curr.root.length;i++)
    {
        ...
        edges.add(JSON.stringify({id:`${child.concept_id}-root-${curr.concept_id}`,from:child.concept_id,to:curr.concept_id,isRoot:true}))
